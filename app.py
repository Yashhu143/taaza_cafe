...
# Initialize MySQL database safely (deferred if connection details are offline or missing)
try:
    import db
    db.init_db()
except Exception as db_err:
    print(f"Warning: Database initialization deferred/failed: {db_err}")
    ...
    if 'image' in request.files:
        ...
        if file and file.filename and allowed_file(file.filename):
            ...
            try:
                ...
                print(f"Warning: File upload failed due to write restrictions on serverless host: {write_err}")
